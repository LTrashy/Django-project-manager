...
def projects(request):
    ...
    if request.method == "GET":
        ...
        # except ValueError:
        #     return render(
        #         request, "projects.html", {"error": "Error listando projectos"}
        #     )
...
def userStories(request, id):
    if request.method == "GET":
        ...
        userStories = UserStory.objects.filter(project=project)
        return render(
            request, "userStories.html", {"userStories": userStories, "project": id}
        )
    else:
        try:
            print(request)
            ...
            return render(
                request,
                "userStories.html",
                {"error": "Error creando historia de usuario"},
            )
...
def tickets(request, userStory):
    ...
    if request.method == "GET":
        ...
        try:
            ...
            ticket.save()
            return redirect("listTickets", listTickets=userStory.id)
        except ValueError:
...
def update_tickets(request, id):
    ...
    if request.method == "GET":
        ...
        try:
            ...
            ticket.save()
            return redirect("listTickets", listTickets=ticket.userStory.id)
        except ValueError:
            return render(
                request, "update_tickets.html", {"error": "Error actualizando ticket"}
            )
...
def list_tickets(request, listTickets):
    try:
        userStory = get_object_or_404(UserStory, id=listTickets)
        tickets = Ticket.objects.filter(userStory=userStory)
        return render(
            request,
            "list_tickets.html",
            {"tickets": tickets, "userStory": userStory},
        )
    except ValueError:
...
def history_tickets(request):
    try:
        ...
        return render(
            request, "history_tickets.html", {"error": "Error listando ticket"}
        )
